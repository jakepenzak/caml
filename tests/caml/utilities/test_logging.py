"""Tests for caml.utilities.logging module."""

import logging

import pytest

from caml.utilities.logging import (
...
class TestVerbosityMap:
    def test_map_values(self):
        assert _VERBOSITY_MAP == {0: logging.WARNING, 1: logging.INFO, 2: logging.DEBUG}
...
class TestGetSectionHeader:
    def test_contains_title(self):
        assert "Title" in _get_section_header("Title")

    def test_separator_char(self):
        assert "-" in _get_section_header("T", sep_char="-")
        assert "=" not in _get_section_header("T", sep_char="-")

    def test_custom_width(self):
        lines = _get_section_header("T", width=20).strip().split("\n")
        assert len(lines[0]) == 20

    def test_auto_width(self):
        title = "Hello"
        lines = _get_section_header(title).strip().split("\n")
        assert len(lines[0]) == len(title) + 5

    def test_structure(self):
        lines = _get_section_header("T").strip().split("\n")
        assert len(lines) == 3
