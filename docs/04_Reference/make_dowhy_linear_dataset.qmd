...
```python
extensions.synthetic_data.make_dowhy_linear_dataset(
    beta=2.0,
    n_obs=1000,
    n_confounders=10,
...
|------------|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| df         | pandas.DataFrame        | The generated dataset where y is the outcome, d are the treatment(s), X are the covariates that are utilized for heterogeneity only, and W are the confounders. |
| true_cates | dict\[str, np.ndarray\] | The true conditional average treatment effects for each treatment.                                                                                              |
| true_ate   | dict\[str, float\]      | The true average treatment effect for each treatment.                                                                                                           |
